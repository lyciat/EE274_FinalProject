...
import tempfile
import unittest
from scl.compressors.huffman.lz77 import LZ77Decoder, LZ77Sequence, LZ77StreamsEncoder, LZ77StreamsDecoder
...
class HashBasedMatchFinder(MatchFinderBase):
    ...
    def __init__(
        ...
        self.repcodes=collections.deque(maxlen=3)
        self.repcodes.extend([1, 4, 8])
        # TODO: change here
        self.repcodes_on=False
    ...
    def find_best_match(self, lookahead_buffer):
        ...
        # first check the repcodes to try to do the fastest matching
        if self.repcodes_on:
            for code in self.repcodes:
                best_literals_count, best_match_pos, best_length = self.find_best_match_at_position(
                lookahead_buffer, code)
                if best_length >= self.minimum_match_length:
                    self.repcodes.appendleft(code)
                    if not self.lazy:
                        return (best_literals_count, best_match_pos, best_length)
                    else:
...
def test_sherlock_holmes_encode_decode():
    """full test for LZ77SlidingWindowEncoder and LZ77SlidingWindowDecoder
    ...
    # test lossless compression
    assert try_file_lossless_compression(
        "sherlock_ascii.txt", encoder, decoder, encode_block_size=1000
    )

def test_github_encode_decode():
    """full test for LZ77SlidingWindowEncoder and LZ77SlidingWindowDecoder
    ...
    # test lossless compression
    assert try_file_lossless_compression(
        "joined_github_data.txt", encoder, decoder, encode_block_size=1000
    )

def test_random_ascii_encode_decode():
    """full test for LZ77SlidingWindowEncoder and LZ77SlidingWindowDecoder
    ...
    # test lossless compression
    assert try_file_lossless_compression(
        "random_ascii.txt", encoder, decoder, encode_block_size=1000
    )
