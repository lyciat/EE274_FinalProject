...
"""
# 1 - huffman, 2 - rans, 3 - tans, 4 - arithmetic
...
from scl.core.prob_dist import ProbabilityDist, Frequencies
...
import math
from scl.compressors.huffman.rANS import rANSEncoder, rANSDecoder, rANSParams
from scl.compressors.huffman.tANS import tANSEncoder, tANSDecoder, tANSParams
from scl.compressors.huffman.arithmetic_coding import ArithmeticEncoder, ArithmeticDecoder, AECParams
import copy
import numpy as np
...
class EmpiricalrANSEncoder(DataEncoder):
    """Perform rANS encoding of the values and return the encoded bitarray.

    We apply rANS coding for the values and also store the frequency dictionary to enable the
    decoder to decode the values.
    """
    ...
    def encode_block(self, data_block: DataBlock):
        ...
        # first encode the values with empirical rANS code
        counts = DataBlock(vals).get_counts()
        if len(counts) > 0: 
            rans_param = rANSParams(Frequencies(counts))
            values_encoding = rANSEncoder(rans_param).encode_block(DataBlock(vals))
...
class EmpiricalrANSDecoder(DataDecoder):
    def __init__(self, alphabet_size):
        self.alphabet_size = alphabet_size
    ...
    def decode_block(self, encoded_bitarray: BitArray):
        ...
        rans_param = rANSParams(counts)

        # load the encoding of the values
        rans_encoding_size = bitarray_to_uint(
            encoded_bitarray[num_bits_consumed : num_bits_consumed + ENCODED_BLOCK_SIZE_HEADER_BITS]
        )
        num_bits_consumed += ENCODED_BLOCK_SIZE_HEADER_BITS
        # decode values with Huffman
        decoded_vals, num_bits_consumed_vals = rANSDecoder(rans_param).decode_block(
            encoded_bitarray[num_bits_consumed : num_bits_consumed + rans_encoding_size]
        )
        assert rans_encoding_size == num_bits_consumed_vals
        num_bits_consumed += rans_encoding_size
        return decoded_vals, num_bits_consumed
    
class EmpiricaltANSEncoder(DataEncoder):
    """Perform tANS encoding of the values and return the encoded bitarray.

    We apply tANS coding for the values and also store the frequency dictionary to enable the
    decoder to decode the values.
    """
    ...
    def encode_block(self, data_block: DataBlock):
        ...
        # first encode the values with empirical tANS code
        counts = DataBlock(vals).get_counts()
        if len(counts) > 0: 
            # get counts to the closest power of two:
            curr_sum = sum(counts.values())
            getting_closer = True
            i = -1
            if curr_sum > 0:
                dist = np.abs(1 - curr_sum)
                while getting_closer == True:
                    i += 1
                    pow_2 = 2 ** i
                    curr_dist = np.abs(pow_2 - curr_sum)
                    if curr_dist < dist:
                        dist = curr_dist
                    elif curr_dist > dist:
            ...
            # pow_2 is now the nearest power of 2
            # add dist to the largest value of the dictionary to solve the pow_2 bug
            # print(counts)
            max_key = max(counts, key=counts.get)
            if pow_2 < curr_sum:
                difference = abs(dist - counts[max_key]) / counts[max_key]
                # print("difference: ", difference)
                if difference > 0.3:
                    # print("subtracting")
                    counts[max_key] = int(counts[max_key] - dist)
                else:
                    # print("adding anyways") 
                    pow_2 = 2 ** i
                    dist = np.abs(pow_2 - curr_sum)
                    counts[max_key] = int(counts[max_key] + dist)
            ...
            # print("Closes Power of 2: ", pow_2)
            # print("Distance: ", dist)
            # print("Key to add to: ", max_key)
            # print("New Sum: ", sum(counts.values()))

            tans_param = tANSParams(Frequencies(counts))
            values_encoding = tANSEncoder(tans_param).encode_block(DataBlock(vals))
...
class EmpiricaltANSDecoder(DataDecoder):
    def __init__(self, alphabet_size):
        self.alphabet_size = alphabet_size
    ...
    def decode_block(self, encoded_bitarray: BitArray):
        ...
        num_bits_consumed += ENCODED_BLOCK_SIZE_HEADER_BITS
        # decode values with Huffman
        decoded_vals, num_bits_consumed_vals = tANSDecoder(tans_param).decode_block(
            encoded_bitarray[num_bits_consumed : num_bits_consumed + rans_encoding_size]
        )
        assert rans_encoding_size == num_bits_consumed_vals
        num_bits_consumed += rans_encoding_size
...
class EmpiricalArithmeticEncoder(DataEncoder):
    """Perform tANS encoding of the values and return the encoded bitarray.
    ...
    def encode_block(self, data_block: DataBlock):
        ...
        if len(counts) > 0: 
            ...
            values_encoding = ArithmeticEncoder(params, freq_model_enc).encode_block(DataBlock(vals))
...
class EmpiricalArithmeticDecoder(DataDecoder):
    def __init__(self, alphabet_size):
        self.alphabet_size = alphabet_size
    ...
    def decode_block(self, encoded_bitarray: BitArray):
        ...
        freq = Frequencies(counts)
        params = AECParams()
        freq_model_dec = AdaptiveIIDFreqModel(
            freq, max_allowed_total_freq = params.MAX_ALLOWED_TOTAL_FREQ
        )
        ...
        num_bits_consumed += ENCODED_BLOCK_SIZE_HEADER_BITS
        # decode values with Huffman
        decoded_vals, num_bits_consumed_vals = ArithmeticDecoder(params, freq_model_dec).decode_block(
            encoded_bitarray[num_bits_consumed : num_bits_consumed + rans_encoding_size]
        )
        assert rans_encoding_size == num_bits_consumed_vals
        num_bits_consumed += rans_encoding_size
...
class LogScaleBinnedIntegerEncoder(DataEncoder):
    ...
    def __init__(self, offset=0, max_num_bins=32):
        ...
        if compression_scheme == 1:
            ...
            self.empirical_huffman_encoder = EmpiricalrANSEncoder(alphabet_size=self.max_num_bins)
        elif compression_scheme == 3:
            self.empirical_huffman_encoder = EmpiricaltANSEncoder(alphabet_size=self.max_num_bins)
        elif compression_scheme == 4:
            self.empirical_huffman_encoder = EmpiricalArithmeticEncoder(alphabet_size=self.max_num_bins)
...
class LogScaleBinnedIntegerDecoder(DataDecoder):
    ...
    def __init__(self, offset=0, max_num_bins=32):
        ...
        if compression_scheme == 1:
            ...
            self.empirical_huffman_decoder = EmpiricalrANSDecoder(alphabet_size=self.max_num_bins)
        elif compression_scheme == 3:
            self.empirical_huffman_decoder = EmpiricaltANSDecoder(alphabet_size=self.max_num_bins)
        elif compression_scheme == 4:
            self.empirical_huffman_decoder = EmpiricalArithmeticDecoder(alphabet_size=self.max_num_bins)
